...
// Fetch rejected jobs from the rejected_jobs table with search filtering
$query = "SELECT * FROM rejected_jobs";
if (!empty($searchTerm)) {
    $query .= " WHERE name LIKE '%" . mysqli_real_escape_string($con, $searchTerm) . "%' 
                OR email LIKE '%" . mysqli_real_escape_string($con, $searchTerm) . "%'
                OR phone LIKE '%" . mysqli_real_escape_string($con, $searchTerm) . "%'
                OR position LIKE '%" . mysqli_real_escape_string($con, $searchTerm) . "%'
                OR address LIKE '%" . mysqli_real_escape_string($con, $searchTerm) . "%'";
}
...
<head>
   ...
   <style>
        ...
        .btn-success {
            background-color: #28a745; /* Green for "reject" */
            color: white;
        }
...
<body>
    ...
    <div class="container">
        <h2 class="text-center">rejected Job Applications</h2>
